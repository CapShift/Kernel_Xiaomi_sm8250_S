...
#ifdef CONFIG_CPUFREQ_GOV_SCHEDUTIL_SINGLE_PERF
	return min(util, effective_cpu_util(sg_cpu->cpu, cpu_util_cfs(sg_cpu->cpu),
					  FREQUENCY_UTIL, NULL));
#else
...
#ifndef CONFIG_CPUFREQ_GOV_SCHEDUTIL_SINGLE_PERF
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTIL_PLUS
...
static void sugov_get_util_hook(void *unused, struct sugov_cpu *sg_cpu, unsigned long util)
{	
	util = sugov_get_util(sg_cpu);
}

static void sugov_start_hook(void *unused, struct cpufreq_policy *policy, void (*uu)(struct update_util_data *data, u64 time, unsigned int flags))
{	
	if (policy_is_shared(policy))
		uu = sugov_update_shared;
	else if (policy->fast_switch_enabled && cpufreq_driver_has_adjust_perf())
...
{
	register_trace_cpufreq_get_util_hook(sugov_get_util_hook, NULL);
	register_trace_cpufreq_start_hook(sugov_start_hook, NULL);
}
