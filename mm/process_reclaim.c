		...
		unsigned long arg)
{
	...
	if (!access_ok(write ? VERIFY_WRITE : VERIFY_READ, (void *)arg, sizeof(struct process_reclaim_cmd))) {
	...
	if (!access_ok(write ? VERIFY_WRITE : VERIFY_READ, (void *)cmd_entry.cmd, iovec_size)) {
...
static const struct file_operations process_reclaim_fops = {
	.compat_ioctl	= process_reclaim_ioctl,
};
